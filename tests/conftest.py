...
Rationale:
- Guardian code and tests are implemented with asyncio-native primitives.
...
- Allow explicit multi-backend runs via GUARDIAN_TEST_ALL_BACKENDS=1.
"""
...
def anyio_backend(request) -> str:
    ...
    Optional: set GUARDIAN_TEST_ALL_BACKENDS=1 to run asyncio and trio.
    """
    if os.getenv("GUARDIAN_TEST_ALL_BACKENDS") == "1":

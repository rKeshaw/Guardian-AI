  ...
  root.innerHTML = `
    <section class="card">
      <div class="report-header">
        <div><div class="logo glitch" style="font-size:1rem">AEGIS AI</div><h3>SECURITY ASSESSMENT REPORT</h3></div>
        <div>
...
function bindDownloadButtons(sessionId) {
  ...
  document.getElementById("dl-html").onclick = () => downloadFile(`/api/v1/scan/${sessionId}/report/html`, `aegis-report-${sessionId}-${date}.html`);
  document.getElementById("dl-md").onclick = () => downloadFile(`/api/v1/scan/${sessionId}/report/markdown`, `aegis-report-${sessionId}-${date}.md`);
  document.getElementById("dl-json").onclick = () => downloadFile(`/api/v1/scan/${sessionId}/report/json`, `aegis-report-${sessionId}-${date}.json`);
}

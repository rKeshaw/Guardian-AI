...
from aegis.api.graph_viz import build_graph_response
...
async def lifespan(_app: FastAPI):
    logger.info("Aegis initialising …")
    await initialize_aegis_components()
    yield

app = FastAPI(
    title="Aegis — Multi-Agent Penetration Testing System",
...
async def _wait_for_ollama(max_attempts: int = 5) -> bool:
    ...
    from aegis.core.ai_client import ai_client
    delay = 5.0
...
async def initialize_aegis_components() -> bool:
    ...
    try:
        ...
        logger.info("Aegis components initialised successfully")
        return True
...
async def dashboard():
    ...
    port = os.environ.get("AEGIS_PORT", "8888")
    return f"""<!DOCTYPE html>
<html>
<head><title>Aegis</title>
<style>
...
async def export_report_html(session_id: str):
    ...
    if not results:
        raise HTTPException(404, detail="Results not found")
    from aegis.core.report_renderer import render_html_report
...
async def export_report_markdown(session_id: str):
    ...
    if not results:
        raise HTTPException(404, detail="Results not found")
    from aegis.core.report_renderer import render_markdown_report
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=int(os.environ.get("AEGIS_PORT", "8888")))

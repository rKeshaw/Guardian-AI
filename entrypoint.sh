...
# aegis user (uid 1000), then drops privileges and starts the application.
#
# Why this is necessary:
#   Docker bind mounts (./data:/app/data, ./logs:/app/logs) are owned by the
#   host user that created them — usually root. The aegis user inside the
...
chown -R aegis:aegis /app/data /app/logs 2>/dev/null || true
chmod 755 /app/data /app/logs 2>/dev/null || true

echo "[entrypoint] Starting Aegis as aegis user ..."
exec gosu aegis "$@"